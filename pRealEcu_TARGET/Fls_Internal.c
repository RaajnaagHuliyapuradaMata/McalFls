...
#if(FLS_FLASH_ACCESS == FLS_CODEFLASH_ACCESS) || (FLS_FLASH_ACCESS == FLS_CFDF_ACCESS)
#include "fcl_cfg.hpp"
#endif
...
#if(FLS_FLASH_ACCESS == FLS_DATAFLASH_ACCESS) || (FLS_FLASH_ACCESS == FLS_CFDF_ACCESS)
#include "r_fdl_global.hpp"
#endif

#if(FLS_AR_VERSION == FLS_AR_HIGHER_VERSION)
#define FLS_INTERNAL_C_AR_RELEASE_MAJOR_VERSION FLS_AR_RELEASE_MAJOR_VERSION_VALUE
#define FLS_INTERNAL_C_AR_RELEASE_MINOR_VERSION FLS_AR_RELEASE_MINOR_VERSION_VALUE
#define FLS_INTERNAL_C_AR_RELEASE_REVISION_VERSION FLS_AR_RELEASE_REVISION_VERSION_VALUE
...
#if(FLS_AR_VERSION == FLS_AR_HIGHER_VERSION)
#if(FLS_INTERNAL_AR_RELEASE_MAJOR_VERSION != FLS_INTERNAL_C_AR_RELEASE_MAJOR_VERSION)
  #error "Fls_Internal.c : Mismatch in Release Major Version"
#endif

#if(FLS_INTERNAL_AR_RELEASE_MINOR_VERSION != FLS_INTERNAL_C_AR_RELEASE_MINOR_VERSION)
  #error "Fls_Internal.c : Mismatch in Release Minor Version"
#endif

#if(FLS_INTERNAL_AR_RELEASE_REVISION_VERSION != FLS_INTERNAL_C_AR_RELEASE_REVISION_VERSION)
...
FUNC(r_fdl_status_t, FLS_PRIVATE_CODE)Fls_DFProcessReadCommand(void){
  ...
  if(Fls_GVar.Fls_GulCurrentLength > LulMaxReadBytes)
  {
    LulFDLLength = (LulMaxReadBytes / FLS_FOUR);
      Fls_GVar.Fls_GulCurrentLength = Fls_GVar.Fls_GulCurrentLength - LulMaxReadBytes;
      Fls_GVar.Fls_GulRequestedLength = Fls_GVar.Fls_GulRequestedLength - (LulMaxReadBytes - Fls_GVar.Fls_GucOffset);
    Fls_GVar.Fls_GblProcessContinue = FLS_TRUE;
  }
  else{
  ...
  do
  {
...
#if((FLS_DEV_ERROR_DETECT == STD_ON) && (FLS_TIMEOUT_MONITORING == STD_ON))
   while((LpFdlReqCmd->status_enu == R_FDL_BUSY) && (Fls_GulTimeout > FLS_ZERO));
#else
   ...
   else if(R_FDL_ERR_BLANKCHECK == LenFDLStatus)
   {
      LpReadAddress = (volatile uint32 *)(&(LpFdlReqCmd->idx_u32));
         LenFDLStatus = R_FDL_FCUFct_ReadOperation(
            LpReadAddress,
            (uint32)Fls_GVar.pTempBufferAddress,
            LulWordCnt);
      ...
      Fls_GVar.pTempBufferAddress = Fls_GVar.pTempBufferAddress + R_FDL_WRITE_SIZE;
    LulFDLLength--;
      Fls_GVar.Fls_GulReadAddress = Fls_GVar.Fls_GulReadAddress + R_FDL_WRITE_SIZE;
   }while((LulFDLLength > FLS_ZERO ) && ((LenFDLStatus == R_FDL_OK) || (LenFDLStatus == R_FDL_ERR_ECC_SED)));
  if(R_FDL_ERR_ECC_DED == LenFDLStatus)
  {
      Fls_GVar.Fls_GulSrcDestAddress = Fls_GVar.Fls_GulReadAddress - R_FDL_WRITE_SIZE;
  }
...
FUNC(void, FLS_PRIVATE_CODE)Fls_EndJobProcess(void){
  ...
  if(FLS_COMMAND_ERASE == Fls_GVar.Fls_GenCommand)
  {
   if(FLS_DF_ACCESS == Fls_GVar.Fls_AccessType)
   {
      ...
      {
#if((FLS_DEV_ERROR_DETECT == STD_ON) && (FLS_BLANK_CHECK == FDL_SUPPORTED))
...
#if(FLS_FLASH_ACCESS == FLS_DATAFLASH_ACCESS) || (FLS_FLASH_ACCESS == FLS_CFDF_ACCESS)
#if((FLS_BLANKCHECK_API == STD_ON) || ((FLS_DEV_ERROR_DETECT == STD_ON) && (FLS_BLANK_CHECK == FDL_SUPPORTED)))
  ...
  {
#if(FLS_FLASH_ACCESS == FLS_DATAFLASH_ACCESS)
      if((R_FDL_OK == Fls_GstVar.GucDFStatus) || (R_FDL_ERR_ECC_SED == Fls_GstVar.GucDFStatus))
    #elif(FLS_FLASH_ACCESS == FLS_CODEFLASH_ACCESS)
   ...
   if(FLS_TRUE == LblFlashStatus)
   {
      ...
      if(FLS_TRUE == Fls_GVar.Fls_GblProcessContinue)
      {
        ...
        if(FLS_DF_ACCESS == Fls_GVar.Fls_AccessType)
        {
          LulBytesProcessed = (LulBytesProcessed - Fls_GVar.Fls_GucOffset);
               Fls_GVar.pTempBufferAddress = (Fls_GVar.pTempBufferAddress - LulBytesProcessed);
        }
        else{
        }
#endif
        ...
        if(FLS_DF_ACCESS == Fls_GVar.Fls_AccessType)
        {
               Fls_GVar.pTempBufferAddress = (Fls_GVar.pTempBufferAddress + Fls_GVar.Fls_GucOffset) - LulBytesProcessed;
          LulBytesProcessed = Fls_GVar.Fls_GulRequestedLength;
        }
        else{
        }
      ...
      if(FLS_DF_ACCESS == Fls_GVar.Fls_AccessType)
      {
        ...
        {
               *((uint8*)Fls_GVar.pBufferAddress) = *((uint8*)Fls_GVar.pTempBufferAddress);
          LddLoopCount++;
          Fls_GVar.pBufferAddress++;
          Fls_GVar.pTempBufferAddress++;
        }
...
#if(FLS_FLASH_ACCESS == FLS_CODEFLASH_ACCESS) || (FLS_FLASH_ACCESS == FLS_CFDF_ACCESS)
        Fls_GVar.Fls_GulReadAddress = Fls_GVar.Fls_GulReadAddress + LulBytesProcessed;
#endif
      }
  ...
  else if(FLS_COMMAND_COMPARE == Fls_GVar.Fls_GenCommand)
  {
   ...
   {
#if(FLS_FLASH_ACCESS == FLS_DATAFLASH_ACCESS) || (FLS_FLASH_ACCESS == FLS_CFDF_ACCESS)
         if((FLS_DF_ACCESS == Fls_GVar.Fls_AccessType) && (R_FDL_ERR_ECC_SED == Fls_GstVar.GucDFStatus))
      {
        if(NULL_PTR != Fls_GpConfigPtr->pEccSEDNotificationPointer)
        {
      ...
      if(FLS_TRUE == Fls_GVar.Fls_GblProcessContinue)
      {
        ...
        {
          LulBytesProcessed = (LulBytesProcessed - Fls_GVar.Fls_GucOffset);
               Fls_GVar.pTempBufferAddress = (Fls_GVar.pTempBufferAddress + Fls_GVar.Fls_GucOffset);
        }
        else{
        }
#endif
      ...
      while(LddLoopCount < LulBytesProcessed)
      {
            if(*(Fls_GVar.pBufferAddress) != *(Fls_GVar.pTempBufferAddress))
        {
...
#if(FLS_FLASH_ACCESS == FLS_DATAFLASH_ACCESS) || (FLS_FLASH_ACCESS == FLS_CFDF_ACCESS)
                  Fls_GVar.pTempBufferAddress = Fls_GVar.pTempBufferAddress - (Fls_GVar.Fls_GucOffset + LulBytesProcessed);
                  Fls_GVar.Fls_GulSrcDestAddress = Fls_GVar.Fls_GulSrcDestAddress + (Fls_GVar.Fls_GucOffset + LulBytesProcessed);
            Fls_GVar.Fls_GucOffset = FLS_ZERO;
#endif
          }
          else{
  ...
  else if(FLS_COMMAND_WRITE == Fls_GVar.Fls_GenCommand)
  {
   ...
   if((R_FDL_OK == Fls_GstVar.GucDFStatus) || (R_FCL_OK == Fls_GstVar.GucCFStatus))
#endif
   {
      LblFlashStatus = FLS_TRUE;
   }
